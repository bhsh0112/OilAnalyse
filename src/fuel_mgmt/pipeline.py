...
from .stations import annotate_stations, spatial_effect
...
def run(data: Path, out_dir: Path, skip_llm: bool = False, skip_poi: bool = False) -> dict[str, Path]:
    ...
    raw_events = detect_refuel_events(df, sigma)
    if skip_poi:
        events = score_refuel_events(df, raw_events)
        spatial: dict = {}
    else:
        annotated = annotate_stations(df, raw_events, cache_path=out_dir / "gaode_cache.json")
        events = score_refuel_events(df, annotated)
        spatial = spatial_effect(events)
    stops = long_stops(df)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--skip-poi", action="store_true", help="不检索周边加油站")
